{ desktop, hostname, inputs, lib, username, kernel, ... }: {
  imports = [
  ...
  ++ lib.optional (builtins.pathExists (./. + "/common/users/${username}")) ./common/users/${username} # user specific configuration that isnt possible using home manager (for example setting default shell)
